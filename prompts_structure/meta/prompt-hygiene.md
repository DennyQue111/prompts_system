...
同一信息在 prompt 中出现两次 = 噪音。重复不仅浪费 token 权重，还会让模型困惑"到底哪个为准"。

### 8.1 正向与反向不得重复

❌ 正面写了 `非3D渲染、非真实感、非矢量扁平`，反向又写 `3D渲染，真实皮肤，矢量扁平`
→ 模型被同一组概念正反各刺激一次，结果不可预测。

✅ 正面写 `polished 2D anime key visual illustration, 手绘质感，完成度高，非3D渲染`，反向只保留正面未覆盖的具体排除项：`文字，水印，多余人`

### 8.2 模块间不得重复

❌ 【画面主体】写 `黑色短发被风吹得微微飘动`，【环境动态】又写 `黑色短发被风吹得微微飘向一侧`
→ 同一动作在两处各占一次权重，挤压了其他描述的空间。

✅ 风动写在【环境动态】，【画面主体】只写角色姿态与表情，不重复描述风的效应。

### 8.3 风格词汇一次到位

❌ 开头写 `polished 2D anime key visual illustration`，中间又写 `NOT sketch, NOT rough draft, NO plastic gloss`
→ 风格描述被否定词切割成碎片，锚定力量分散。

✅ 风格锚定集中在 prompt 最前面一次说完：`polished 2D anime key visual illustration, finished digital painting, clean ink outlines, flat cel-shading, NO sketch quality, NO 3D render`。后续不再出现任何风格相关否定词。
