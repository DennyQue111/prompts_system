## Description
Use this architecture when the user uploads any reference image and wants to **extract a character** from it, then generate a renderer-neutral concept-character prompt that reproduces the character — preserving face, hairstyle, body type, and the source's intentional art direction.

The core problem this solves: reference images contain visual identity and rendering evidence that pure text-to-image prompts struggle to reproduce. This structure bridges that gap by systematically deconstructing the source's visual DNA and translating it into renderer-neutral prompt language.

**Input:** Reference image + user instruction (e.g., "提取画面中心的人物", "取左边穿夹克的那个人", "要这个人带着她的护手和围巾")

**Output:** A character prompt in `core-t2i.md` format, but filled with observed (not imagined) content, plus style preservation keywords.
...
This is the most critical section. The source image has a specific rendering style that the selected renderer must approximate. Break it down into the following dimensions:
...
This string will be prepended to the final prompt to anchor the style.
...
### Step 5: Renderer-Neutral Output Prompt

The final prompt, formatted for the supported image renderers image generation. Follow the panel layout defined in `simple_layout_instruction.md` in this directory (default), but fill ALL content from Steps 1-4 observations and inferences.
...
2. **Style analysis is mandatory.** A character prompt without style keywords will produce the renderer default aesthetic, which will NOT match the MJ reference. Always complete Step 2 fully.
...
| Pitfall | Fix |
|---------|-----|
| Style analysis skipped → output looks generic | Always complete Step 2 — the style keywords are what make the reproduction possible |
...
- **When to use `core-t2i.md` instead**: User describes a character from imagination/script — no reference image provided.
