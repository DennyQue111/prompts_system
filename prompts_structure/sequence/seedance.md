...
5. **参考图说清用途就不再重复——但当视频模型无法自动匹配合成图标签时，需在逐镜建立对应关系。** 前面写清楚每张图管什么后，画面内容直接描述画面本身。但如果生成结果与参考图完全无关，说明模型未能建立"镜头 ↔ 参考图区域"的对应关系，此时需在逐镜画面内容开头用括号标注对应区域（如"（对应合成图 CUT 1 区域）"），或在参考图描述中将每个区域单独列出并命名。
...
| 字段 | 必填 | 写法 |
|------|------|------|
| **画面内容** | ✅ | 直接描述画面，不引用参考图编号；如需建立与参考图的精确对应，可在开头用括号标注对应区域（如"（对应合成图 CUT 1 区域）"） |
| **运镜** | ✅ | 从选项列表中选一个 |
