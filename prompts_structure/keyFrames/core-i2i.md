...
1. **Reference image anchoring with explicit boundaries**: Character references and environment references serve fundamentally different purposes. The prompt must reproduce character appearance exactly (identity lock) while extracting only visual DNA from environment references. Never apply a generic "extract DNA from all references" instruction to character images.
...
3. **Frame aspect-ratio enforcement**: Multi-frame generators often default to square cells. Each keyframe must be explicitly locked to 16:9 landscape (1.77:1).
...
- **Reference boundaries** (critical): For each reference image, state what it anchors and what it does NOT anchor. This prevents the renderer from treating a character reference as mere "style DNA" instead of exact identity.
...
Multi-frame generators tend to default to square cells when given an open layout. Every layout instruction must explicitly define how the 16:9 aspect ratio is preserved — either by making the frame small within a larger cell, or by splitting each cell into "frame area + margin area."
...
Each keyframe frame uses the same five-field structure; renderer-specific word choice belongs in the selected Adapter:
...
**Adapter word choice:** apply only the selected renderer Adapter; do not append another model's rules.
...
After completing the sequence content, apply the selected renderer Adapter. It owns vocabulary for material cleanliness, reference-artifact handling, negative constraints, and panel-divider behavior; do not duplicate one renderer's rules in this Core.
...
5. **Apply one renderer Adapter after the Core**: It adds only the quality and exclusion controls required by the selected renderer.
6. **Never transfer reference artifacts**: Compression artifacts, banding, micro-pattern noise, annotations, and watermarks are pollution, not style.
...
11. Read the selected renderer Adapter for final language, quality, and negative-constraint rules.
