## Description
Use this architecture when generating a **single cinematic frame for a supported image renderer** — a standalone shot that functions as a storyboard cell rendered at full quality. This is the atomic unit of visual storytelling: one composition, one moment, one emotional beat.
...
The Style Palette is the **visual DNA** of a project — a set of material, color, and camera decisions that remain consistent across every frame. Supported image renderers understand natural language descriptions of film texture, format, color science, and camera language well — use them directly in the prompt.
...
The Style Palette is appended as the final block of the content prompt. Unlike Midjourney, supported image renderers do not use `--` parameter syntax — describe everything in natural language:
...
- **Movement implication:** the supported image renderers handle motion blur descriptions reasonably well — "mid-stride," "hair caught in wind," "rain frozen mid-fall," "motion blur on passing cars" all work.
...
- **Style Palette is a first-class section.** supported image renderers strength is understanding natural language descriptions of film texture and color — lean into this.
- **No CG anime workaround needed.** supported image renderers have different moderation paths than Chinese platforms (即梦/豆包).
