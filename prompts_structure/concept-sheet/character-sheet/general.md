...
- **Main Visual** — top-left, 1/3 width × ~4/5 height. Full-body shot of the character in default outfit, neutral/resting expression, with name/label. **Must show personality through body language** — the stance, hand position, and subtle physical tells should communicate who this person is without words. A loyal character stands slightly forward-weighted; a gentle giant has relaxed half-curled fingers; a battle-hardened survivor has old scars on resting knuckles.
- **Right Upper — View Variations** — upper half of the right area. The same character shown from 2–3 different viewing angles (front, side, back, 3/4 perspective), showing the outfit and build from multiple directions. Back view should reveal the character's silhouette width and how clothing drapes across the shoulders — these are personality-layered details.
- **Right Lower — Expressions / 喜怒哀乐** — lower half of the right area. Four shoulder-up or face-only panels in a 2×2 grid: **喜 (joy)**, **怒 (anger)**, **哀 (sorrow)**, **乐 (happiness)**. Face-only is preferred when the user provides detailed facial feature descriptions; shoulder-up when body language matters to the emotion. **Emotions should match the character's personality** — a stoic character's anger is cold and quiet (jaw tight, eyes narrowed, no shouting), while a hot-headed character's anger is explosive (brows sharply down, mouth open, veins visible).
...
| **Panel 1: Main visual** | Full body, default outfit, neutral face, richest description | Complete visual description — facial features, hair, build/body type, **body language as personality manifestation (stance, hand state, weight distribution)**, full outfit from outer to inner to shoes, any visible accessories, default expression, standing posture. The outfit should be written with its "prototype" whenever possible (e.g., "combat suit adapted from a wedding tuxedo"). This is the largest rendered area; the richest description lives here. |
...
| **Panel 3: Expressions** | 喜/怒/哀/乐, 2×2 grid, shoulder-up or face-only | Each expression: (a) the emotion label, (b) key facial changes — eyebrow position, eye shape, mouth, any wrinkles or tension areas, (c) how the eyes specifically change (e.g., "eyes crinkle at corners" for joy, "brows drawn down and together" for anger). **Match expression personality to the character** — a stoic's anger is cold and tight, not explosive. Face detail from the character's facial description should be referenced here. |
| **Panel 4: Bottom row** | All wearable items + body marks, one row, AI-scaled | Each item in its own panel — coat/jacket, shirt, pants, shoes, ring, necklace, scar close-up, birthmark location. **Clothing items should show the "prototype garment" origin** (e.g., a wedding suit jacket's lapel still visible beneath tactical modifications). For marks: a cropped body close-up with the mark visible. More items → smaller panels, fewer items → larger panels. |
...
## Usage Notes
- **Personality first, description second**: Before writing anything, translate the character's inner traits into the 4 dimensions of Personality-to-Visual (body language, hand state, signature gesture, contrast shot). These visual cues should saturate the main visual panel.
- **Layout first**: The grid instruction must come first. Go macro (zones) → micro (panel contents).
- **Main visual is the anchor**: ~4/5 height × 1/3 width is the hero panel. This paragraph should contain every visual signal the model needs to render the character accurately — including personality manifested in the body.
- **The prototype rule for outfits**: "combat suit adapted from [original garment]" is always better than "black tactical suit." A character who died in a wedding suit and now wears it as battle armor is instantly more memorable.
